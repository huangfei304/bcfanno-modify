...
const struct MolecularConsequenceTerms MCT[] = {
    ...
    // splice variant
    { "exonic_splice_region_variant",               "SpliceSite", },
    ...
    { "noncoding_transcript_splice_region_variant", "SpliceSite", },
    // UTR variant
    ...
    { "intronic_splice_region_variant",               "SpliceSite", },
};
...
static int noncoding_transcript_update_molecular_conseqeunce_state(struct mc_handler *h, struct mc *mc, struct mc_inf *inf, struct mc_type *type, struct gea_record *v, char *ref, int lref, char *alt, int lalt)
{
    ...
    // check if variantion located in the splice sites around the edge of utr and cds regions    
        ...
        else if( inf->offset >-splice_site_options.acceptor_region ) type->con1 = mc_intron_splice_sites;
    }
    
    else if ( inf->offset > 0 ) {
        type->con1 = mc_coding_intron;
        if ( inf->offset < splice_site_options.range ) type->con1 = mc_splice_donor;
        else if( inf->offset < splice_site_options.donor_region ) type->con1 = mc_intron_splice_sites;
    }
            ...
            *loc = c->utr5_length - *pos +1;
            ...
            if (*offset) { 
                ...
                *func = func_region_utr5_intron;
                ...
                else if ( *offset < 0 && *offset >-splice_site_options.acceptor_region ) *con_splice = mc_intron_splice_sites;
                else if ( *offset >0 && *offset < splice_site_options.range ) *con_splice = mc_splice_donor;
                else if ( *offset >0 && *offset < splice_site_options.donor_region ) *con_splice = mc_intron_splice_sites;                
            }
            else {
...
static int transcript_molecular_consequence_update(struct mc_handler *h, struct mc *n, struct mc_core *trans, struct gea_record *v)
{
    ...
    if ( (type->con2 == mc_splice_donor || type->con2 == mc_splice_acceptor || type->con2 == mc_noncoding_splice_region || type->con2 == mc_intron_splice_sites ) && (type->con1==mc_noncoding_intron || type->con1==mc_coding_intron || type->con1==mc_utr3_intron || type->con1==mc_utr3_intron)){
	int t = type->con1;
	type->con1 = type->con2;
	type->con2 = t;
    }
    ...
    if ( n->start != n->end ) {
        ...
        else if ( con1 !=mc_unknown && type->con2 == mc_unknown ) type->con2 = con1;
    ...
    if (is_coding) {
        inf->aa_length = (v->c.cds_length - v->c.utr5_length)/3 - 1;
    }
...
static int transcripts_variant_state_update(struct mc *n, struct mc_handler *h, int a, int c)    
{
    ...
    for ( i = h->i_record, j = 0; i < h->n_record && j < c; ++i,++j ) {
	    ...
	    if (trans->type.con1 == mc_noncoding_intron || trans->type.con1 == mc_coding_intron || trans->type.con1 == mc_utr3_intron || trans->type.con1 == mc_utr5_intron ) trans->type.count = v->blockCount-trans->type.count-1;
        ...
        if ( type->loc_amino == type->loc_end_amino ) {
            ...
            if ( type->n ) {
                if (type->con1 == mc_conservation_inframe_insertion ) {
                  inframe_insertion:
                    ksprintf(str, "(p.%s%d_%s%dins", codon_names[type->ori_amino], type->loc_amino, codon_names[type->ori_amino], type->loc_end_amino);
                    int i;
                    for ( i = 0; i < type->n; ++i ) kputs(codon_names[type->aminos[i]], str);
                    kputc('/', str);
                    ksprintf(str, "p.%s%d_%s%dins", codon_names[type->ori_amino], type->loc_amino,codon_names[type->ori_amino], type->loc_end_amino);
...
static void generate_hgvsnom_string_exonLoss(struct mc *mc, struct mc_type *type, struct mc_inf *inf, kstring_t *str)
{
	...
	if (mc->type == var_type_ins){
            if (inf->strand == '-' ) ksprintf(str, "%s:n.%d", inf->transcript, inf->loc+strlen(mc->alt)-1);
            else ksprintf(str, "%s:n.%d", inf->transcript, (inf->loc+1));
        }else ksprintf(str, "%s:n.%d", inf->transcript, inf->loc);
        if ( inf->offset > 0 ) ksprintf(str, "+%d", inf->offset);
        else if (inf->offset < 0 ) ksprintf(str, "%d", inf->offset);
    }    
    else if ( type->func1 == func_region_cds || type->func1 == func_region_intron) {
  	if (mc->type == var_type_ins){
            if (inf->strand == '-' ) ksprintf(str, "%s:c.%d", inf->transcript, inf->loc+strlen(mc->alt)-1);
        ...
        if (type->func1 == func_region_outrange && type->func2 == func_region_utr5_exon ){
            if(mc->type == var_type_del || mc->type == var_type_delins ){
                ksprintf(str,"%s:c.-%d_-%d",inf->transcript,inf->end_loc+strlen(mc->ref)-1,inf->end_loc);
                if( mc->type == var_type_del ) ksprintf(str,"del%s",mc->ref);
                else ksprintf(str, "delins%s",mc->ref);
            }else generate_hgvsnom_string_empty(str);
        }else generate_hgvsnom_string_empty(str);
    ...
    if ( type->func2 == func_region_noncoding_exon || type->func2 == func_region_noncoding_intron ||
        ...
        if (mc->type == var_type_ins){
            ...
            }else ksprintf(str, "%d", inf->end_loc+strlen(mc->alt)-1);
         }else ksprintf(str, "%d", inf->end_loc);
        if ( inf->end_offset > 0 ) ksprintf(str, "+%d", inf->end_offset);
        else if (inf->end_offset < 0 ) ksprintf(str, "%d", inf->end_offset);        
    }
    ...
    if (mc->type == var_type_ins ) ksprintf(str,"ins%s",mc->alt);
    else ksprintf(str,"del%s",mc->alt);
}
